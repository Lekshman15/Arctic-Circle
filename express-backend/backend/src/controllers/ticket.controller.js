...
async function raiseTicket(req, res) {
  ...
  if (!appliance || !address || !preferredTimings) {
    return res.status(400).json({ error: "appliance, address and preferredTimings are required." });
  }
...
async function submitFeedback(req, res) {
  ...
  if (ticket.userId !== req.user.id) {
    return res.status(403).json({ error: "This isn't your ticket." });
  }
  if (ticket.status !== "COMPLETED") {
    return res.status(400).json({ error: "Feedback can only be left once the ticket is closed." });
  }
